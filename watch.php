...
// Obtenemos TODOS los comentarios y los ordenamos por fecha descendente
$sql_comments = "SELECT c.*, u.username, u.avatar 
                 FROM comments c 
                 JOIN users u ON c.user_id = u.id 
                 WHERE c.video_id = $video_id 
                 ORDER BY c.created_at ASC"; // Orden ASC para que las respuestas salgan en orden
...
// Ordenar comentarios principales (nivel 0) por fecha DESCENDENTE (más nuevos arriba)
if (isset($comments_by_parent[0])) {
    usort($comments_by_parent[0], function($a, $b) {
...
function renderComments($parentId = 0, $level = 0, $comments_by_parent) {
    ...
    foreach ($comments_by_parent[$parentId] as $comment) {
        ...
        // Margen para anidación visual
        // Nivel 0 = 0px, Nivel 1+ = 48px (para no hacer escalera infinita visualmente)
        ...
        <div class="comment-item" id="comment-<?php echo $comment['id']; ?>" style="margin-left: <?php echo $marginLeft; ?>;">
            ...
            <div class="comment-body">
                ...
                <!-- CONTENIDO SIN ESPACIOS EXTRA NI nl2br (CSS white-space: pre-wrap se encarga) -->
                ...
                <div class="comment-actions-toolbar">
                    ...
                    <!-- BOTÓN RESPONDER (Solo hasta nivel 2 de profundidad para evitar anidación excesiva) -->
                    <?php if ($level < 2): ?>
